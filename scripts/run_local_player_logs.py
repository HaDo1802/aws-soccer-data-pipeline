...
def main() -> None:
    ...
    try:
        ...
        total_rows = sum(len(payload["player_stats"]) for payload in payloads)
        print(f"Players scraped: {len(payloads)}")
        print(f"Total rows scraped: {total_rows}")
    except Exception as exc:
        LOGGER.exception("Top-level player log run failed: %s", exc)
